...
- `.browserslistrc` - This file contains instructions to other modules like
  ...
  Currently, GregSiteTheme is supporting the last 4 versions (`last 4 versions`) of
...
GregSiteTheme now offers an easy way to inject inline SVG's into your CSS. In order
...
```scss
.c-quote {
  background-color: var(--greg_site_theme-color-background);
  padding: var(--greg_site_theme-space-inset);
}
...
.c-quote__icon * {
  fill: var(--greg_site_theme-color-accent);
}
...
.c-quote__text {
  ...
  p {
    font-size: inherit;
    font-style: italic;
    font-weight: var(--greg_site_theme-font-weight-bold);
    quotes: '' '\201d';
    text-indent: 0;
...
.t-gray-white {
  --greg_site_theme-color-background: var(--greg_site_theme-color-gray-95);
  --greg_site_theme-color-background-hover: var(--greg_site_theme-color-gray-80);
  --greg_site_theme-color-text: var(--greg_site_theme-color-gray-30);
  --greg_site_theme-color-text-link: var(--greg_site_theme-color-orange-30);
  --greg_site_theme-color-border: var(--greg_site_theme-color-gray-70);
}
...
with `--greg_site_theme` This $property-prefix variable must also be set in
...
example, `--greg_site_theme-color-blue-200` or `--greg_site_theme-color-orange-500`.
...
as such. For example, `--greg_site_theme-color-background` or
`--greg_site_theme-color-text`. Semantic colors can very depending on the context in
...
Since version 4.x, GregSiteTheme ships with an accessible menu that works with
Drupal's default menu system out of the box. To utilize for the main menu there
are a few steps you will need to take:

By default the Main navigation menu provided by Drupal is used by GregSiteTheme. To
...
GregSiteTheme takes the `menu--main.html.twig` file in `templates/menu` directory
